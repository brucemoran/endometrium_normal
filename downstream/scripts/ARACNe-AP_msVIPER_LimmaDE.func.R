#
runAracneAP <- function(datasets, signatures, TAG, BASEDIR, OUTDIR, nosub=NULL){
  ...
  #    if(is.null(nosub)){
  #      system(paste0("sh ", OUTDIR, "/bootstraps/", TAG, ".ARACNe_AP_run.sh"))
  #    }
  # }
}

runMsViper <- function(combined_clin_res, TAG, BASEDIR, OUTDIR, DERES, minconx=NULL){
  ...
  ##so mv to new file specifies consolidate has fully completed
  ...
  for (pp in 1:length(qvals)){
    ...
    de.results <- DERES
    de.padj.results <- de.results %>% dplyr::filter(adj.P.Val < 0.01)
    nodenms <-  as.vector(unlist(de.padj.results$external_gene_name[de.padj.results$external_gene_name %in% names(mrsldgList)]))
